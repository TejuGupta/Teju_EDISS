...
var recordBlock = 80; //The number of records to write at once.
...
lineReader.eachLine('./Project4Data/ProductRecords.json', function(line, last) {
  ...
  try{
    ...
    values += `('${jsonRecord.title}', '${jsonRecord.description}','${jsonRecord.categories[0]}','${jsonRecord.asin}')`;
    numRecords++;

//****************************************************Change the query to align with your schema******************************************************/
    if (numRecords == recordBlock) {
      query = `INSERT INTO productdata(productName,productDescription,\`group\`,asin) VALUES ${values};`; //Template, replaces ${values} with the value of values.
      values = "";
      numRecords = 0;
      execute = true;
      console.log(query);
